// This is a modified version of the xls file xls/common/logging/vlog_is_on.cc
...
#include "cryptopals/util/internal/logging/errno_saver.h"

ABSL_FLAG(int32, v, 0,
          "Show all VLOG(m) messages for m <= this. Overridable by "
          "--vmodule.");
...
std::atomic<int32> absl::logging_internal::vlog_epoch{1};
...
int SetVLOGLevel(absl::string_view module_pattern, int log_level) {
  ...
  for (const VModuleInfo* info = vmodule_list.load(std::memory_order_relaxed);
    ...
    } else if (!found && absl::logging_internal::SafeFNMatch(
  ...
  ::absl::logging_internal::vlog_epoch.fetch_add(1, std::memory_order_release);
...
int InitVLOG(std::atomic<int32>* site, absl::string_view full_path) {
  ...
  while (info != nullptr) {
    if (info->module_is_path) {
      ...
      if (absl::logging_internal::SafeFNMatch(info->module_pattern, stem)) {
        new_site = Site(info->vlog_level.load(std::memory_order_acquire),
                        global_epoch);
        break;
      }
    } else if (absl::logging_internal::SafeFNMatch(info->module_pattern,
                                                   stem_basename)) {
