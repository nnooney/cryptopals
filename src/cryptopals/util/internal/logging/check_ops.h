// This is a modified version of the xls file xls/common/logging/check_ops.h
...
#ifndef CRYPTOPALS_UTIL_INTERNAL_LOGGING_CHECK_OPS_H_
#define CRYPTOPALS_UTIL_INTERNAL_LOGGING_CHECK_OPS_H_
...
#include "cryptopals/util/internal/logging/null_guard.h"

#ifdef NDEBUG
// `NDEBUG` is defined, so `DCHECK_EQ(x, y)` and so on do nothing.  However,
// we still want the compiler to parse `x` and `y`, because we don't want to
// lose potentially useful errors and warnings.
#define LOGGING_INTERNAL_DCHECK_NOP(x, y)    \
  while (false && ((void)(x), (void)(y), 0)) \
  ::absl::logging_internal::NullStream().stream()
#endif

#define LOGGING_INTERNAL_CHECK(failure_message) \
  ::absl::logging_internal::LogMessageFatal(__FILE__, __LINE__, failure_message)
#define LOGGING_INTERNAL_QCHECK(failure_message)                       \
  ::absl::logging_internal::LogMessageQuietlyFatal(__FILE__, __LINE__, \
                                                   failure_message)

#define LOGGING_INTERNAL_CHECK_OP(name, op, val1, val2)                        \
  while (                                                                      \
      std::string* logging_internal_check_op_result ABSL_ATTRIBUTE_UNUSED =    \
          ::absl::logging_internal::name##Impl(                                \
              ::absl::logging_internal::GetReferenceableValue(val1),           \
              ::absl::logging_internal::GetReferenceableValue(val2),           \
              LOGGING_INTERNAL_STRIP_STRING_LITERAL(#val1 " " #op " " #val2))) \
  LOGGING_INTERNAL_CHECK(*logging_internal_check_op_result).stream()
#define LOGGING_INTERNAL_QCHECK_OP(name, op, val1, val2)                       \
  while (                                                                      \
      std::string* logging_internal_qcheck_op_result =                         \
          ::absl::logging_internal::name##Impl(                                \
              ::absl::logging_internal::GetReferenceableValue(val1),           \
              ::absl::logging_internal::GetReferenceableValue(val2),           \
              LOGGING_INTERNAL_STRIP_STRING_LITERAL(#val1 " " #op " " #val2))) \
  LOGGING_INTERNAL_QCHECK(*logging_internal_qcheck_op_result).stream()
...
// Helper functions for `LOGGING_INTERNAL_CHECK_OP` macro family.  The
...
// type.
#define LOGGING_INTERNAL_CHECK_OP_IMPL(name, op)                         \
  template <typename T1, typename T2>                                    \
...
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_EQ, ==)
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_NE, !=)
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_LE, <=)
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_LT, <)
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_GE, >=)
LOGGING_INTERNAL_CHECK_OP_IMPL(Check_GT, >)
#undef LOGGING_INTERNAL_CHECK_OP_IMPL
...
#endif  // CRYPTOPALS_UTIL_INTERNAL_LOGGING_CHECK_OPS_H_
