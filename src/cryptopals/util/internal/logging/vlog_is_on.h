// This is a modified version of the xls file xls/common/logging/vlog_is_on.h
...
// conditional logging.
//
// It's used by VLOG and VLOG_IF in logging.h to trigger the logging.
//
...
#ifndef CRYPTOPALS_UTIL_INTERNAL_LOGGING_VLOG_IS_ON_H_
#define CRYPTOPALS_UTIL_INTERNAL_LOGGING_VLOG_IS_ON_H_
...
#include "cryptopals/util/internal/integral_types.h"
...
// int32 at every VLOG_IS_ON() call site.  The level determines
...
#define VLOG_IS_ON(verbose_level)                    \
  (::absl::logging_internal::VLogEnabled(            \
      []() -> std::atomic<int32>* {                  \
        static std::atomic<int32> site__(            \
            ::absl::logging_internal::kDefaultSite); \
        return &site__;                              \
      }(),                                           \
...
class VLogSite {
 public:
  ...
  // Returns true if logging is enabled.  Like VLOG_IS_ON(2), this uses
...
#endif  // CRYPTOPALS_UTIL_INTERNAL_LOGGING_VLOG_IS_ON_H_
