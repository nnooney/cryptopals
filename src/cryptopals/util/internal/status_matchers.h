...
// iree/base/internal/status_matchers.h obtained from
...
#ifndef CRYPTOPALS_UTIL_INTERNAL_STATUS_MATCHERS_H_
#define CRYPTOPALS_UTIL_INTERNAL_STATUS_MATCHERS_H_
...
// absl::status and that the contained T value matches another matcher.
template <typename T>
...
// Implements a gMock matcher that checks whether a absl::status container (e.g.
...
// IsOkMatcherGenerator is an intermediate object returned by absl::IsOk().
// It implements implicit type-cast operators to supported matcher types:
// Matcher<const absl::Status &> and Matcher<const absl::StatusOr<T> &>. These
// typecast operators create gMock matchers that test OK expectations on a
// status container.
...
// OK absl::status and for the contained T object to match |value_matcher|.
//
// Example:
//
...
// matchers test that a given absl::status container has an OK status.
...
#define EXPECT_OK(rexpr) EXPECT_THAT(rexpr, ::testing::status::IsOk())
#define ASSERT_OK(rexpr) ASSERT_THAT(rexpr, ::testing::status::IsOk())
...
#endif  // CRYPTOPALS_UTIL_INTERNAL_STATUS_MATCHERS_H_
