...
// xls/common/status/status_builder.cc obtained from
...
#include "cryptopals/util/internal/symbolized_stacktrace.h"
...
void StatusBuilder::ConditionallyLog(const absl::Status& status) const {
  ...
  switch (rep_->logging_mode) {
    ...
    case Rep::LoggingMode::kVLog: {
      ...
      struct LogSites {
        absl::Mutex mutex;
        std::unordered_map<const void*, absl::VLogSite> sites_by_file
  ...
  const std::string maybe_stack_trace =
      rep_->should_log_stack_trace
          ? absl::StrCat("\n", absl::GetSymbolizedStackTraceAsString(
