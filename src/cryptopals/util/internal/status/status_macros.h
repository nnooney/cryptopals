// This is a modified version of the xls file xls/common/status/status_macros.h
...
#ifndef CRYPTOPALS_UTIL_INTERNAL_STATUS_STATUS_MACROS_H_
#define CRYPTOPALS_UTIL_INTERNAL_STATUS_STATUS_MACROS_H_

#include "absl/base/optimization.h"
#include "cryptopals/util/internal/source_location.h"
#include "cryptopals/util/internal/status/status_builder.h"  // IWYU pragma: export
...
#define RETURN_IF_ERROR(expr)                                   \
  ...
  if (::absl::status_macro_internal::StatusAdaptorForMacros     \
...
// the status and is returned directly in manner similar to RETURN_IF_ERROR.
...
#define STATUS_MACROS_IMPL_ASSIGN_OR_RETURN_(statusor, lhs, rexpr,           \
  ...
  if (ABSL_PREDICT_FALSE(!statusor.ok())) {                                  \
    ::absl::StatusBuilder _(std::move(statusor).status(), ABSL_LOC);         \
  ...
  {                                                                          \
    static_assert(                                                           \
        #lhs[0] != '(' || #lhs[sizeof(#lhs) - 2] != ')' ||                   \
            !::absl::status_macro_internal::HasPotentialConditionalOperator( \
                #lhs, sizeof(#lhs) - 2),                                     \
...
#endif  // CRYPTOPALS_UTIL_INTERNAL_STATUS_STATUS_MACROS_H_
